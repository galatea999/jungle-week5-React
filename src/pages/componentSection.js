...
const LEARN_ITEMS = [
  '컴포넌트가 큰 화면을 작은 조각으로 나누는 이유와, 그렇게 나누면 무엇이 편해지는지 이해한다.',
  '함수형 컴포넌트가 props라는 입력을 받아 화면 조각을 만들어 내는 과정을 살펴본다.',
  '부모 컴포넌트가 자식 컴포넌트에게 데이터를 내려주는 흐름을 익힌다.',
  '같은 컴포넌트를 여러 번 재사용해도 코드가 깔끔하게 유지되는 이유를 확인한다.',
];

const COMPONENT_EXPLANATION =
  'React에서 컴포넌트는 화면을 만드는 가장 기본 단위입니다. 큰 페이지를 한 번에 다 그리기보다, 버튼, 카드, 목록, 프로필처럼 반복해서 쓰이는 부분을 작은 부품처럼 나눠 두면 읽기 쉽고 고치기도 쉬워집니다. 함수형 컴포넌트는 이런 부품을 함수로 표현한 것으로, 어떤 값을 입력받으면 어떤 화면 조각을 만들어 낼지 정리해 두는 역할을 합니다. 결국 컴포넌트를 잘 나누는 것은 화면을 예쁘게 만드는 기술이라기보다, 코드를 오래 관리하기 쉽게 만드는 기술에 가깝습니다.';

const COMPONENT_IMPORTANCE =
  'props는 부모 컴포넌트가 자식 컴포넌트에게 건네주는 정보라고 생각하면 쉽습니다. 같은 ProfileCard를 두 번 써도 이름과 직업이 다르게 보이는 이유는 컴포넌트 자체가 두 종류라서가 아니라, 같은 컴포넌트에 다른 props를 넣었기 때문입니다. 그래서 이 챕터의 핵심은 "모양을 만드는 것은 컴포넌트, 내용을 채우는 것은 props"라는 역할 분리를 이해하는 데 있습니다. 이 감각이 잡히면 뒤에서 배우는 state나 Hook도 훨씬 덜 어렵게 느껴집니다.';
